...
<!-- Table creating section -->
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . $row["target_id"] . "<td>" . $row["name"] . "<td>" . " " . $row["first_mission"]. "<td>" . " " . $row["type"]. "<td>" . " " . $row["no_mission"]. "<td>" . "<a href='targets_update_db_form.php?id=$row[target_id]'>". "edit"?>
        <td><a href="targets_delete_db.php?id=<?php echo $row["target_id"]?>" onclick="return confirm('Are you sure?')">Delete</a></td>
        <?php
      }
    }
