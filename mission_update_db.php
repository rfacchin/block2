...
<div class="container mt-3">
    ...
    if ($result->num_rows > 0) {
      ...
      while ($row = $result->fetch_assoc()) {
        echo "<tr>";
        echo "<td>" . $row["mission_id"] . "<td>" . $row["destination"] . "<td>" . " " . $row["launch_date"]. "<td>" . " " . $row["type"]. "<td>" . " " . $row["crew_size"]. "<td>" . " " . $row["target_id"]."<td>". "<a href='mission_update_db_form.php?id=$row[mission_id]'>". "edit"?>
        <td><a href="mission_delete_db.php?id=<?php echo $row["mission_id"]?>" onclick="return confirm('Are you sure?')">Delete</a></td>
